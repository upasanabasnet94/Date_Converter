...
class datenepali:
    ...
    start_english = datetime.date(1943, 4, 14)
    end_english = datetime.date(2034, 4, 13)
    start_nepali = "2000/01/01"

    def converttonepali(self, engdate):
        yearsd = 2000
        monthd = 1

        engdate = datetime.date(int(engdate[0:4]), int(engdate[5:7]), int(engdate[8:]))
        dayslapsed = (engdate - datenepali.start_english).days
        for item in datenepali.nepali_date:
            for months in range(1, 13):
                if dayslapsed >= item[months]:
                    dayslapsed = dayslapsed - int(item[months])
                    monthd = months + 1
                    yearsd = int(item[0])
                    if monthd == 13:
                        monthd = 1
                        yearsd = yearsd + 1
        return str(yearsd) + '/' + str(monthd) + '/' + str(dayslapsed + 1)
